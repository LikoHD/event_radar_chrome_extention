...
首次切入 A/B Test 自动开始采集，主动暂停后切换仍保持暂停。监控、刷新、清空和导出固定底部，AI 进度和错误位于 AI 解读子页。ab-research.js 按域名匹配插件内 ab-research.json 的 16 条历史调研，与当前脱敏参数、上报、疑似线索和页面 HTML 一并送入模型；历史证据不增加当前计数。

### 2026-09-05 推送后文档同步
**Scope:** panel、service worker、manifest、ab-*、AI 设置。
**Before:** 仅记录 1.1.3 分类跳转及直接发送。
**After:** 同步 1.1.4 默认监控、底部操作区、AI 子页状态和调研目录；保留原生脚本与 MV3 结构。
**Reason:** 与已推送源码保持一致。
**Impact:** 45 项自动化测试及压缩版浏览器回归通过；没有商店发布。
